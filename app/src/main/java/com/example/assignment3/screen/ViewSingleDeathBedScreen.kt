...
fun ViewSingleDeathBedScreen(id: String) {
    ...
    Column(modifier = Modifier.padding(10.dp), horizontalAlignment = Alignment.CenterHorizontally) {
        Text(deathBed!!.fullName)
        Text("Cause of death: ${deathBed.causeOfDeath.name}")

        // Makes sure the image takes the whole width, since not images are of the same size
        Image(
            painterResource(id = deathBed.causeOfDeath.image),
            contentDescription = deathBed.causeOfDeath.name,
